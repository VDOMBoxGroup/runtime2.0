...
class VDOM_acl_manager(object):
    ...
    def __init__(self):
        self.acl = managers.storage.read_object(
            VDOM_CONFIG["ACL-MANAGER-STORAGE-RECORD"])
        if (self.acl is None):
            self.acl = {}
            self.sync()
    ...
    def sync(self):
        ...
        managers.storage.write_object_async(
            VDOM_CONFIG["ACL-MANAGER-STORAGE-RECORD"], self.acl)
    ...
    def __allow(self, object_id):
        ...
        if hasattr(request, "request_type") and request.request_type in ["vdom", "action"]:
            ...
            if app and (app.id == object_id or app.objects.get(object_id)):
                return True
...
# ## PUBLIC INTERFACE ###
    ...
    def add_access(self, object_id, username, access_type):
        ...
        self.__set_rule(object_id, username, access_type)
