...
class VDOM_user_manager(object):
    ...
    def __init__(self):
        """Initialize"""
        self.users_by_name = {}
        self.users = managers.storage.read_object(
            VDOM_CONFIG["USER-MANAGER-STORAGE-RECORD"])
        if self.users is None:
            self.users = {}
        try:
            self.root_user = self.get_user_by_id(managers.storage.read(
                VDOM_CONFIG["USER-MANAGER-ROOT-ID-STORAGE-RECORD"]))
            if not self.root_user:
                raise Exception()
        except Exception:
            cf = VDOM_config()
            self.root_user = self.create_user(
                "root", cf.get_opt("ROOT-PASSWORD"), system=True)
            self.root_user.member_of.append("ManagementLogin")
            managers.storage.write_async(
                VDOM_CONFIG["USER-MANAGER-ROOT-ID-STORAGE-RECORD"], self.root_user.id)
        try:
            self.guest_user = self.get_user_by_id(managers.storage.read(
                VDOM_CONFIG["USER-MANAGER-GUEST-ID-STORAGE-RECORD"]))
            if not self.guest_user:
                raise Exception()
        except Exception:
            self.guest_user = self.create_user("guest", "", system=True)
            managers.storage.write_async(
                VDOM_CONFIG["USER-MANAGER-GUEST-ID-STORAGE-RECORD"], self.guest_user.id)
        # create hash
    ...
    def __check_system(self):
        ...
        sys_groups = [("ManagementLogin", ("Allows to log into the management area"), [
                       "root", "Admin"])]
    ...
    def create_user(self, login, password, name1="", name2="", email="", slv="", system=False):
        ...
        if (self.name_exists(login)):
            raise VDOM_exception(
                ("Login %s already exists in the system" % login))
        user = VDOM_user()
        user.id = vdomid()
        user.login = login
    ...
    def create_group(self, name, descr="", system=False):
        ...
        if (self.name_exists(name)):
            raise VDOM_exception(
                ("Name %s already exists in the system" % name))
    ...
    def sync(self):
        ...
        managers.storage.write_object_async(
            VDOM_CONFIG["USER-MANAGER-STORAGE-RECORD"], self.users)
