...
class VDOM_environment(object):
    ...
    def __init__(self, headers, handler):
        ...
        self.__environment["SERVER_ADDR"] = str(socket.gethostbyname(
            socket.gethostname()))  # str(handler.server.server_address[0])

        hh = ""
        if "HTTP_HOST" not in self.__environment:
            hh = self.__environment["SERVER_ADDR"]
        else:
            hh = self.__environment["HTTP_HOST"]
        hh = hh.split(":")[0]
        self.__environment["HTTP_HOST"] = hh
        self.__environment["SERVER_PORT"] = str(
        ...
        try:
            self.__environment["QUERY_STRING"] = request_list[1] if len(
                request_list) > 1 else ""
        except Exception:
            self.__environment["QUERY_STRING"] = ""
