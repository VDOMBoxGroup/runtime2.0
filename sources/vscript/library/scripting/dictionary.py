...
class scripting_dictionary(generic):
    ...
    def v_item(self, key, **keywords):
        if "let" in keywords:
            ...
            try:
                return self._items[key.subtype]
            except KeyError:
                return v_empty
    ...
    def v_key(self, key, **keywords):
        if "let" in keywords:
            try:
                self._items[keywords["let"].subtype] = self._items[key.as_simple]
            except KeyError:
                raise errors.element_not_found
        elif "set" in keywords:
            try:
                self._items[keywords["set"].subtype] = self._items[key.as_complex]
            except KeyError:
                raise errors.element_not_found
        else:
            raise errors.object_has_no_property
