...
def inc_day(year, month, day, cnt):
    ...
    while day not in range(1, ymMax+1):
        if day > 0:
            day -= ymMax
            month += 1
            if month > 12:
...
def inc_second(year, month, day, hour, minute, second, cnt):
    ...
    second += cnt
    if second not in range(1, 61):
        mindiff, second = second // 60, second % 60
        year, month, day, hour, minute = inc_minute(
            year, month, day, hour, minute, mindiff)
...
def v_dateserial(year, month, day):
    ...
    if year < 100:
        year += 1900  # TODO: Update to close support modern years
    if month < 1 or month > 12:
        year, month, day = inc_month(year, month, 1, 0)
...
def v_datepart(interval, value, firstdayofweek=None, firstweekofyear=None):
    ...
    if interval == u"yyyy":
        ...
        if count == 0:
            count = get_week_count(value, firstweekofyear, year - 1, 12, 31)
...
def v_dateadd(interval, number, value):
    ...
    if interval == u"yyyy":
        ...
        year, month, day, hour, minute, second = inc_second(
