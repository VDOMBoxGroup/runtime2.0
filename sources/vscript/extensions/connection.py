...
class v_proxy(generic):
    ...
    def __call__(self, *arguments, **keywords):
        if "let" in keywords:
            ...
            if len(arguments) > 1:
                raise errors.wrong_number_of_arguments
            return self._value[arguments[0].as_string]
    ...
    def build_handler(self, proxy):
        ...
        if http_proxy:
            ...
            password_manager.add_password(
                None, http_proxy, self._username, self._password)
...
class v_connection(generic):
    ...
    def v_encoding(self, **keywords):
        if "let" in keywords:
            ...
            try:
                self._encoding, self._codec = encoding, codecs.lookup(encoding)
            except LookupError:
                pass
        elif "set" in keywords:
    ...
    def v_read(self):
        ...
        try:
            data = self._value.read()
        except http.client.HTTPException as error:
            raise urllib.error.URLError(error)
        if self._codec:
            try:
                return string(self._codec.decode(data))
            except UnicodeError:
                raise errors.invalid_procedure_call(name=u"read")
        else:
            return binary(data)
    ...
    def v_write(self, data):
        ...
        if self._codec:
            try:
                data = self._codec.encode(data.as_string)
            except UnicodeError:
                raise errors.invalid_procedure_call(name=u"write")
        else:
            data = data.as_binary
        try:
            self._value.write(data)
        except http.client.HTTPException as error:
            raise urllib.error.URLError(error)
        return v_mismatch
