    ...
    def v_firstindex(self, group=None, **keywords):
        if "let" in keywords or "set" in keywords:
            ...
            if group is None:
                ...
                if not isinstance(group, (integer, string)):
                    raise errors.type_mismatch
                try:
                    start = self._match.start(group.value)
                except IndexError:
                    return v_empty
        ...
        if "let" in keywords or "set" in keywords:
            ...
            if group is None:
                ...
                if not isinstance(group, (integer, string)):
                    raise errors.type_mismatch
                try:
                    start, end = self._match.span(group.value)
                except IndexError:
                    return v_empty
        ...
        if "let" in keywords or "set" in keywords:
            ...
            if group is None:
                ...
                if not isinstance(group, (integer, string)):
                    raise errors.type_mismatch
                try:
                    value = self._match.group(group.value)
                except IndexError:
                    return v_empty
...
class v_global_matches(v_matches):
    ...
    def _get_collection(self):
        if self._cache is None:
            self._cache = [
                match for match in self._regexp.finditer(self._value)]
        return self._cache
    ...
    def _get_regex(self):
        if not self._cache:
            self._cache = re.compile(
                self._pattern, re.IGNORECASE if self._ignorecase else 0)
        return self._cache
