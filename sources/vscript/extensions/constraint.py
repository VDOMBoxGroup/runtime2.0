...
from .. import errors
from ..subtypes import generic, string, dictionary, v_empty, v_mismatch
...
class v_problem(generic):
    ...
    def v_addvariables(self, *arguments):
        if len(arguments) < 2:
            ...
            names = tuple(
                name.as_string for name in arguments[0].as_array.items)
        else:
    ...
    def v_addconstraint(self, klass, name, *names):
        ...
        if code.co_argcount < 2:
            raise errors.generic(
                "Constraint function must have at least one argument")
        if names:
    ...
    def v_again(self):
        try:
            ...
            if message and message[0] == "'":
                raise errors.generic(
                    u"Possible variable is not defined: %s" % message.replace("'", "\""))
            else:
