...
class whole_connection_error(whole_error):
    ...
    def __init__(self, url, login, message, line=None):
        whole_error.__init__(self,
                             message=u"Unable to connect to %s as %s (%s)" % (
                                 url, login, message),
                             line=line)
...
class whole_remote_call_error(whole_error):
    ...
    def __init__(self, url, message, line=None):
        whole_error.__init__(self,
                             message=u"Unable to make remote call to %s (%s)" % (
                                 url, message),
                             line=line)
...
def search_for_application_id(name, string):
    document, name = xml.dom.minidom.parseString(
        string.encode("utf-8")), name.lower()
    try:
    ...
    def v_actions(self, **keywords):
        if "let" in keywords or "set" in keywords:
            ...
            if not self._service:
                raise whole_no_connection_error
            try:
                result = self._service.remote("get_server_actions_list", [
    ...
    def v_invoke(self, name, *arguments):
        ...
        try:
            ...
            result = self._service.call(
                self._container, name.as_string, parameter)
        except Exception as error:
            raise whole_remote_call_error(self._url, "%s: %s" % (
                type(error), error)).with_traceback(sys.exc_info()[2])
...
class v_wholeconnection(generic):
    ...
    def v_isconnected(self, **keywords):
        if "let" in keywords or "set" in keywords:
            ...
            try:
                self._service.remote("keep_alive", None, True)
            except Exception:
                return boolean(false)
            return boolean(true)
    ...
    def v_applications(self, name, container=None, **keywords):
        if "let" in keywords or "set" in keywords:
            ...
            if not application:
                raise whole_no_application_error
            try:
                service = VDOM_service.connect(
                    self._url, self._login, self._password, application)
            except Exception as error:
                raise whole_connection_error(self._url, self._login, error)
