...
class v_vdombox(generic):
    ...
    session_id_regex = re.compile(
        r"\<SessionId\>\<\!\[CDATA\[(\S+)\]\]\>\<\/SessionId\>")
    session_key_regex = re.compile(
        r"\<SessionKey\>\<\!\[CDATA\[(\S+)\]\]\>\<\/SessionKey\>")
    hash_string_regex = re.compile(
        r"\<HashString\>\<\!\[CDATA\[(\S+)\]\]\>\<\/HashString\>")
    key_regex = re.compile(r"(\r?\n\<Key\>\d+_\d+\<\/Key\>)$")
    ...
    def v_open(self, address, username, password):
        if self._server is not None:
            raise errors.invalid_procedure_call("open")
        ...
        response = self._server.open_session(
            self._username, hashlib.md5.new(self._password).hexdigest())
        # <Session>
    ...
    def v_close(self):
        if self._server is None:
            raise errors.invalid_procedure_call("close")
    ...
    def v_invoke(self, name, *arguments):
        if self._server is None:
            raise errors.invalid_procedure_call("invoke")
        ...
        try:
            handler = getattr(self._server, name.as_string)
        except AttributeError:
            raise errors.invalid_procedure_call(name=u"invoke")
        arguments = tuple(argument.as_string for argument in arguments)
        response = handler(self._session_id, "%s_%s" %
                           (self._session_key, self._index), *arguments)
        # ...
        # <Key>...</Key>
