...
class VDOM_module_resource(VDOM_module):
    ...
    def getfile(self, application_id, fileurl):
        ...
        if not resource:
            debug("Error reading resource %s:%s" %
                  (str(application_id), str(fileurl)))
            return None
    ...
    def run(self, request_object, request_type):
        ...
        elif ro and getattr(ro, "label", "") == "" and self.ctype in VDOM_module_resource.cached:
            request_object.add_header("Cache-Control", "max-age=86400")

        request_object.add_header(
            "Content-Type", VDOM_module_resource.content_type[self.ctype])
