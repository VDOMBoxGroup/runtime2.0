...
class VDOM_imaging(object):
    ...
    def load(self, application_id, res_id):
        ...
        self.__im = None
        try:
            resource = managers.resource_manager.get_resource(
                application_id, res_id)
            if not resource:
    ...
    def create_font(self, name="arial.ttf", size=12, color="black", fontstyle='normal', fontweight='normal'):
        ...
        try:
            self.__font = ImageFont.truetype(
                VDOM_CONFIG["FONT-DIRECTORY"] + "/" + name + ".ttf", size, 0, encoding="unic")
            # self.__font = self.__font_factory.get_font(name, size, fontstyle = fontstyle, fontweight = fontweight)
        except Exception as e:
            self.__font = ImageFont.truetype(
                VDOM_CONFIG["FONT-DIRECTORY"] + "/arial.ttf", size, 0, encoding="unic")
    ...
    def save_temporary(self, application_id, object_id, label, format="JPEG"):
        ...
        res_id = managers.resource_manager.add_resource(
            application_id, object_id, attributes, output.getvalue())
    ...
    def save(self, application_id, name, format="JPEG"):
        ...
        res_id = managers.resource_manager.add_resource(
            application_id, None, attributes, output.getvalue())
