...
class VDOM_DigestHandler(BaseMiddleware):
    ...
    def __call__(self, environ, start_response):
        if environ["http_authenticator.username"]:
            self._domaincontroller.authDomainUser(
                environ["http_authenticator.realm"], environ["http_authenticator.username"], "", environ)
    ...
    def computeDigestResponse(self, username, realm, password, method, uri, nonce, cnonce, qop, nc):
        ...
        if qop:
            digestresp = self.md5kd(
                md5hA1, nonce + ":" + nc + ":" + cnonce + ":" + qop + ":" + self.md5h(A2))
        else:
            digestresp = self.md5kd(md5hA1, nonce + ":" + self.md5h(A2))

        if not isinstance(self._application, VDOM_DigestHandler):
            self._application = VDOM_DigestHandler(
                self._application, self._domaincontroller)
        return digestresp
...
class VDOM_domain_controller(object):
        ...
        if not davProvider:
            if environ["wsgidav.verbose"] >= 2:
                print("getDomainRealm(%s): '%s'" %
                      (inputRelativeURL, None), file=sys.stdout)
            return None
    ...
    def authDomainUser(self, realmname, username, password, environ):
        ...
        if "dav_user" in session and session["dav_user"] == (self._application.id, obj_id, username, password):
            ...
            if ret:
                session["dav_user"] = (
                    self._application.id, obj_id, username, password)
                return True
            else:
                return False
    ...
    def getDigest(self, realmname, username):
        obj_id = realmname
        if obj_id == '/':
            obj_id = managers.webdav_manager.list_webdav(
                self._application.id)[0]

        session = managers.request_manager.current.session()
        if not session.get("dav_digest"):
            session["dav_digest"] = authGetDigest(
                self._application.id, obj_id, username)
