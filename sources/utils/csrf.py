...
# name of the session variable storing the csrf key
csrf_secret_sess_var_name = 'VDOM_csrf_secret'
# name of the POST argument passing the csrf token
csrf_token_arg_name = 'vdomcsrftoken'
csrf_secret_len = 32
csrf_salt_len = 16
...
def create_csrf_token(salt=''):
    ...
    if not salt:
        salt = binascii.hexlify(get_random_bytes(
            csrf_salt_len)).decode('utf-8')
    h = SHA256.new()
...
def verify_csrf_token(token=''):
    ...
    if not token:
        token = managers.request_manager.get_request(
        ).arguments().arguments().get(csrf_token_arg_name, "")
        if token:
            token = token[0]
