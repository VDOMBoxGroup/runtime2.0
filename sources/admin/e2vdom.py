...
#            debug("INCOMING STATE: %s"%request.last_state["#"])
            ...
            if err:
                if VDOM_CONFIG_1["DEBUG"] == "1":
                    request.write(b"<ERROR>%s</ERROR>" % err.getvalue().replace('&', '&amp;').replace('<', '&lt;').replace('>', '&gt;').replace('"', '&quot;').encode())
                else:
                ...
                request.write(b"<ACTIONS>%s</ACTIONS>" % rr.getvalue().encode("utf-8"))
                request.write(b"<SV>%s</SV>" % json.dumps(request.shared_variables).replace('&', '&amp;').replace('<', '&lt;').replace('>', '&gt;').replace('"', '&quot;').encode())

#           debug("OUTCOMING STATE: %s"%(request.next_state or request.last_state)["#"])
            request.write(b"<STATE value=\"%d\"/>" % (request.next_state or request.last_state)["#"])
