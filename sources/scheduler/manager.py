...
class VDOM_scheduler_manager(object):
    ...
    def restore(self):
        self.__task_list = managers.storage.read_object(
            VDOM_CONFIG["SCHEDULER-MANAGER-INDEX-STORAGE-RECORD"])
    ...
    def add_task(self, task_object, interval):
        ...
        managers.storage.write_object_async(
            VDOM_CONFIG["SCHEDULER-MANAGER-INDEX-STORAGE-RECORD"], self.__task_list)
        crontab = []
        for key in self.__task_list:
